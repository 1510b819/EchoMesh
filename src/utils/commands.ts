export type CommandHandlerProps = {
  ...
  export const handleCommand = ({ message, roomId, setRoomId, setMessages, setMessage }: CommandHandlerProps) => {
    ...
    switch (command) {
      case "/room":
        setMessages((prev) => [...prev, { text: `Current room: ${roomId}`, sender: "System", timestamp: Date.now() }]);
        break;
  
      case "/joinroom":
        if (parts.length < 2) {
          setMessages((prev) => [...prev, { text: "Usage: /joinroom <room_name>", sender: "System", timestamp: Date.now() }]);
        } else {
          const newRoom = parts.slice(1).join(" ");
          setRoomId(newRoom);
          setMessages(() => [{ text: `Joined room: ${newRoom}`, sender: "System", timestamp: Date.now() }]); // Clears chat
        }
        break;
  
      case "/status":
        setMessages((prev) => [...prev, { text: "Status: Online", sender: "System", timestamp: Date.now() }]);
        break;
  
      case "/clear":
        setMessages(() => [{ text: "Chat cleared.", sender: "System", timestamp: Date.now() }]);
        break;
      ...
      case "/me":
        if (parts.length < 2) {
          setMessages((prev) => [...prev, { text: "Usage: /me <message>", sender: "System", timestamp: Date.now() }]);
        } else {
          const actionMessage = parts.slice(1).join(" ");
          setMessages((prev) => [...prev, { text: `*${actionMessage}*`, sender: "Me", timestamp: Date.now() }]);
        }
        break;
  
      case "/whisper":
        if (parts.length < 3) {
          setMessages((prev) => [...prev, { text: "Usage: /whisper <user> <message>", sender: "System", timestamp: Date.now() }]);
        } else {
          const user = parts[1];
          const privateMessage = parts.slice(2).join(" ");
          setMessages((prev) => [...prev, { text: `(whisper to ${user}): ${privateMessage}`, sender: "Me", timestamp: Date.now() }]);
        }
        break;
  
      case "/echo":
        if (parts.length < 2) {
          setMessages((prev) => [...prev, { text: "Usage: /echo <message>", sender: "System", timestamp: Date.now() }]);
        } else {
          const echoMessage = parts.slice(1).join(" ");
          setMessages((prev) => [...prev, { text: echoMessage, sender: "EchoBot", timestamp: Date.now() }]);
        }
        break;
  
      default:
        setMessages((prev) => [...prev, { text: `Unknown command: ${command}. Type /help for a list of commands.`, sender: "System", timestamp: Date.now() }]);
        break;
    }
